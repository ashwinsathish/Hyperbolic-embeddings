...
def create_edgelist(file, df):
    ...
    """
    # loading edges from the (sub)genres themselves
    df1 = (pd
           .read_csv(file,
                     dtype='str'))

    # getting edges from the book descriptions df
    df2 = (df[['title',
               'subclass']]
    ...
    # consistently assigning categories
    df4 = (df3
           .stack()
           .astype('category')
           .unstack())

    # making the categorical values explicit 
...
def combine_edgelists(el1, el2):
    ...
    1st set: from the subgenre info.
    2nd set: from the similarity of the book desc
    """
